...
async function getDonorsById(id) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/program/link/${id}/donors?limit=5&mode=pagination&page=1`,
...
// data fundraiser
async function getProjectSalur(link) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/project?program_link=${link}&limit=1`,
      {
        next: { revalidate: 600 }, // Cache response for 10 minutes (ISR)
        headers: {
          'Cache-Control': 's-maxage=600, stale-while-revalidate', // Efficient API caching
...
export default async function DonasiPage({ params }) {
  ...
  const project = await getProjectSalur(params.id);
